...
class Router {
	init() {
		...
		var loc = Store.Detector.isSafari ? location.hash : window.location.hash
		var hash = loc.split('?')
	...
	updatePageRoute(hash, parts, parent, target) {
		...
		hasher.newHash.type = hasher.newHash.hash == '' ? Constants.HOME : Constants.ABOUT
	...
	sendToDefault() {
		hasher.setHash(Store.defaultRoute())
	}
	setupRoutes() {
		hasher.routes = []
